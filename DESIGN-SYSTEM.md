...
- **Honesty floor extensions shipped:** hero carries `TYP {p50}ms · WORST {p95}ms · 30-SAMPLE BAND`; any feed silence >75s shows `LAST BLOCK Xs AGO` (stale is never rendered as live); `TRAY SPEED` line reports the honest observable — median blockTime→first-sight (bake→tray) — plus the strict `%<1s WITNESSED` figure only when finalize transitions were actually observed (impossible at 4s list cadence vs ~460ms finality; the 600ms sub-poll catches them during congestion).
